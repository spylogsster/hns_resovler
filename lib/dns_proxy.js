...
const { queryTLSA, verifyDANE, formatDANEResult } = require('./dane');
...
// --- DANE verification for proxy ---

/**
 * Run DANE/TLSA verification for a proxied CONNECT request (async, non-blocking).
 * Logs the result but does not affect the tunnel.
 */
async function verifyDANEForProxy(hostname, targetPort, ip, hnsdHost, hnsdPort, quiet) {
  try {
    const tlsaRecords = await queryTLSA(hnsdHost, hnsdPort, hostname, targetPort);
    if (tlsaRecords.length === 0) return;

    const result = await verifyDANE(ip, hostname, targetPort, tlsaRecords);
    const tlsaSummary = tlsaRecords.map(r => `${r.usage} ${r.selector} ${r.matchingType}`).join(', ');
    process.stdout.write(`  [dane]  ${hostname}:${targetPort} TLSA=[${tlsaSummary}] ${formatDANEResult(result)}\n`);
  } catch {
    // TLSA query failed — no DANE records for this domain, not an error
  }
}
...
function startHttpProxy(opts = {}) {
  ...
  const noDane = opts.noDane || false;
  ...
  // Track domains we've already verified to avoid repeated checks
  const daneChecked = new Set();
  ...
  server.on('connect', async (req, clientSocket, head) => {
    ...
    try {
      ...
      // DANE verification (async, non-blocking — doesn't delay the tunnel)
      if (!noDane) {
        const daneKey = `${hostname}:${targetPort}`;
        if (!daneChecked.has(daneKey)) {
          daneChecked.add(daneKey);
          verifyDANEForProxy(hostname, targetPort, ip, hnsdHost, hnsdPort, quiet);
        }
      }
    } catch (e) {
...
function printChromeCommand(port) {
  const platform = os.platform();
  console.log('\nLaunch Chrome with HNS support:\n');
  if (platform === 'win32') {
