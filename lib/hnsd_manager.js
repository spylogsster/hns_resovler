 ...
 * Detects port conflict via exit code 3, kills existing hnsd, and retries.
 * Does NOT attach stderr handlers — leaves stderr stream clean for waitForSync.
 *
...
function startWithRetry(opts = {}) {
  return new Promise((resolve, reject) => {
    ...
    // If hnsd survives 3 seconds, assume it started OK
    ...
    child.on('exit', (code) => {
      ...
      if (code === 3) {
        // Exit code 3 = port binding failure (EFAILURE)
        if (!opts.quiet) {
  ...
  return new Promise((resolve, reject) => {
    ...
    function printProgress() {
      ...
      if (currentHeight > 0) {
        ...
        console.log(`  Connecting to peers... (${peerCount} found)  [${elapsed}s]`);
      }
    }
    ...
    // hnsd writes log output to stdout (not stderr)
    function onData(data) {
      outputBuf += data.toString();
      if (!receivedAnyData) {
        receivedAnyData = true;
        if (showProgress) console.log('  hnsd started, discovering peers...');
      }

      // Process complete lines
      const lines = outputBuf.split('\n');
      outputBuf = lines.pop(); // keep incomplete line

      for (const line of lines) {
        // Count peers
        if (line.includes('addrman: added addr')) peerCount++;
