...
async function verifyDANE(ip, domain, port, tlsaRecords) {
  ...
  for (const tlsa of tlsaRecords) {
    ...
    // DANE-EE (3) and DANE-TA (2) are relevant for HNS (no CA chain)
    // PKIX-EE (1) and PKIX-TA (0) require a valid CA chain — less common for HNS
    if (usage !== 2 && usage !== 3 && usage !== 1 && usage !== 0) {
      continue;
    }
...
function formatDANEResult(result) {
  if (result.verified) {
    return `VERIFIED - certificate matches TLSA record (${result.matchingName}, ${result.usageName})`;
  }
