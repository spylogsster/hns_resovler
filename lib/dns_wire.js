...
function parseRData(buf, off, type, rdlen) {
  switch (type) {
    ...
    case types.TLSA: {
      if (rdlen < 3) return { usage: 0, selector: 0, matchingType: 0, certData: '' };
      const usage = buf[off];
      const selector = buf[off + 1];
      const matchingType = buf[off + 2];
      const certData = buf.subarray(off + 3, off + rdlen).toString('hex');
...
function recordToString(rr) {
  ...
  switch (rr.type) {
    ...
    case types.TLSA: data = `${d.usage} ${d.selector} ${d.matchingType} ${d.certData}`; break;
